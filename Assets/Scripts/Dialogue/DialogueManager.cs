...
public class DialogueManager : MonoBehaviour
{
    ...
    private void FinishTyping()
    {
        ...
        // NOTE: this block was previously missing entirely — DialogueNode had
        // completesTask/triggersDayAdvance fields but nothing ever read them.
        if (currentNode.completesTask && currentNode.taskManager != null && !string.IsNullOrEmpty(currentNode.taskIdToComplete))
        {
            bool wasPrimaryTaskForCurrentDay = currentNode.gameData != null &&
                currentNode.taskIdToComplete == $"day{currentNode.gameData.CurrentDay}_initial";

            Debug.Log($"[DialogueManager] Completing task '{currentNode.taskIdToComplete}'. " +
                      $"Current day: {(currentNode.gameData != null ? currentNode.gameData.CurrentDay.ToString() : "no gameData assigned")}. " +
                      $"Was primary task: {wasPrimaryTaskForCurrentDay}.");

            currentNode.taskManager.CompleteTask(currentNode.taskIdToComplete);

            if (wasPrimaryTaskForCurrentDay && currentNode.advanceSequenceByPrimaryTask)
            {
                if (!string.IsNullOrEmpty(currentNode.targetNpcID))
                {
                    Debug.Log($"[DialogueManager] Setting sequence for npcID '{currentNode.targetNpcID}' to {currentNode.advanceSequenceTarget}.");
                    // Update the specified NPC directly via GameData - works even if that NPC
                    // isn't currently loaded/present in the scene.
                    currentNode.gameData.SetNPCSequence(currentNode.targetNpcID, currentNode.advanceSequenceTarget);
                }
                else if (currentNPC != null)
                {
                    Debug.Log($"[DialogueManager] No targetNpcID set - falling back to current NPC '{currentNPC.npcID}', setting sequence to {currentNode.advanceSequenceTarget}.");
                    // Fallback: no targetNpcID specified, so default to whoever is currently being spoken to.
                    currentNPC.SetSequence(currentNode.advanceSequenceTarget);
                }
            }
            else if (currentNode.advanceSequenceByPrimaryTask)
            {
                Debug.Log($"[DialogueManager] advanceSequenceByPrimaryTask was checked but skipped - " +
                          $"taskIdToComplete ('{currentNode.taskIdToComplete}') did not match the current day's primary task ID.");
            }
        }

        if (currentNode.triggersDayAdvance && currentNode.gameData != null)
        {
            currentNode.gameData.AdvanceDay();
        }
